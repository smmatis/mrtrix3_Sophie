...
#include "dwi/tractography/mapping/mapper_tckeditfixel.h"
...
public:
  ...
  bool operator()(const SetVoxelDir &tangents) {
    // For each voxel tract tangent, assign to a fixel
    std::vector<int32_t> tract_fixel_indices;
    for (SetVoxelDir::const_iterator tangent = tangents.begin(); tangent != tangents.end(); ++tangent) {
      ...
      if (num_fibres > 0) {
        ...
        if (largest_dp > angular_threshold_dp) {
          //add fixel indice to the value tract_fixel_indice (within a voxel)
          tract_fixel_indices.push_back(closest_fixel_index);
          //add 1 to the fixels TDI (count 1 more tract segment for that fixel index)
          fixel_TDI[closest_fixel_index]++;
          //INSTEAD OF DOING ^^ NEED TO FEED THIS BACK TO EDIT THE TRACK//
        }
          //tangent->dir *= largest_dp > angular_threshold_dp;
      }
    }
    return true;
  }
...
void run() {
...

  {
    ...
    const std::string output_path = ("/home/sophie/tckedit_fixel_testing/testingtckout.tck");
    //change to get output_path from argument like in tckedit
