...
//copy of TrackMapperBase to use with tckedit_fixel
class TrackMapperFixels {
...
public:
  ...
  template <class HeaderType>
  TrackMapperFixels(const HeaderType &template_image, const DWI::Directions::FastLookupSet &dirs)
      : info(template_image),
        scanner2voxel(Transform(template_image).scanner2voxel.cast<float>()),
        map_zero(false),
        precise(false),
  ...
  TrackMapperFixels(const TrackMapperFixels &) = default;
  TrackMapperFixels(TrackMapperFixels &&) = default;

  virtual ~TrackMapperFixels() {}
  ...
  bool operator()(const Streamline<> &in, Streamline<> &out) const {
    ...
    vdir.weight = in.weight;
    if (in.empty())
      return true;
    if (preprocess(in, vdir) || map_zero) {
      Streamline<> temp;
      upsampler(in, temp);
      if (precise)
        voxelise_precise(temp, vdir);
      else if (ends_only)
        voxelise_ends(temp, vdir);
      else
        voxelise(temp, vdir);
...
template <class Cont> void TrackMapperFixels::voxelise(const Streamline<> &tck, Cont &output) const {
...
template <class Cont> void TrackMapperFixels::voxelise_precise(const Streamline<> &tck, Cont &out) const {
...
template <class Cont> void TrackMapperFixels::voxelise_ends(const Streamline<> &tck, Cont &out) const {
  if (tck.empty())
    return;
  if (tck.size() == 1) {
...
inline void TrackMapperFixels::add_to_set(SetVoxel &out,
                                        const Eigen::Vector3i &v,
                                        const Eigen::Vector3d &d,
                                        const default_type l) const {
  out.insert(v, l);
}
inline void TrackMapperFixels::add_to_set(SetVoxelDEC &out,
                                        const Eigen::Vector3i &v,
                                        const Eigen::Vector3d &d,
                                        const default_type l) const {
  out.insert(v, d, l);
}
inline void TrackMapperFixels::add_to_set(SetVoxelDir &out,
                                        const Eigen::Vector3i &v,
                                        const Eigen::Vector3d &d,
                                        const default_type l) const {
  out.insert(v, d, l);
}
inline void TrackMapperFixels::add_to_set(SetDixel &out,
                                        const Eigen::Vector3i &v,
                                        const Eigen::Vector3d &d,
                                        const default_type l) const {
  assert(dixel_plugin);
...
inline void TrackMapperFixels::add_to_set(SetVoxelTOD &out,
                                        const Eigen::Vector3i &v,
                                        const Eigen::Vector3d &d,
                                        const default_type l) const {
  assert(tod_plugin);
...
class TrackMapperTWI : public TrackMapperFixels {
public:
  template <class HeaderType>
  TrackMapperTWI(const HeaderType &template_image, const contrast_t c, const tck_stat_t s)
      : TrackMapperFixels(template_image), contrast(c), track_statistic(s) {}

  TrackMapperTWI(const TrackMapperTWI &that)
      : TrackMapperFixels(static_cast<const TrackMapperFixels &>(that)),
